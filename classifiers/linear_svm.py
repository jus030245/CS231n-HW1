...
def svm_loss_naive(W, X, y, reg):
    ...
    num_classes = W.shape[1] #number of the labels
    num_train = X.shape[0] #number of the sample
    loss = 0.0
    for i in range(num_train):
        scores = X[i].dot(W)
        correct_class_score = scores[y[i]] #first get the label of Xi from y[i] and then select the score for its actual label
